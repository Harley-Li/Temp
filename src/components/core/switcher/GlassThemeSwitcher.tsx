import React from 'react';
import { useTheme } from '../../../contexts/ThemeContext';
import styles from './GlassThemeSwitcher.module.scss';

const GlassThemeSwitcher: React.FC = () => {
  const { theme, toggleTheme } = useTheme();

  return (
    <div className={styles.glassThemeSwitcher}>
      <button 
        className={`${styles.switcherContainer} ${styles[theme]}`} 
        onClick={toggleTheme} 
        aria-label={`Swith to ${theme === 'light' ? 'Dark' : 'Light'} mode`}
      >
        <span className={styles.lightOption}>
          <span className={styles.icon}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
              <circle cx="12" cy="12" r="5" fill="currentColor" />
              <path d="M12 3V1M12 23V21M3 12H1M23 12H21M5.6 5.6L4.2 4.2M18.4 18.4L19.8 19.8M5.6 18.4L4.2 19.8M18.4 5.6L19.8 4.2" 
                stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
            </svg>
          </span>
          <span className={styles.text}>Light</span>
        </span>
        
        <span className={styles.darkOption}>
          <span className={styles.icon}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
              <path
                d="M12 22C17.5228 22 22 17.5228 22 12C22 11.5373 21.3065 11.4608 21.0672 11.8568C19.9289 13.7406 17.8615 15 15.5 15C11.9101 15 9 12.0899 9 8.5C9 6.13845 10.2594 4.07105 12.1432 2.93276C12.5392 2.69347 12.4627 2 12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22Z"
              />
            </svg>
          </span>
          <span className={styles.text}>Dark</span>
        </span>
      </button>
    </div>
  );
};

export default GlassThemeSwitcher; 